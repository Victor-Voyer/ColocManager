...
4. La répartition d'une dépense est égale par défaut entre les membres sélectionnés par le créateur. Le payeur n'est pas obligé de faire partie de la répartition : il peut avoir avancé la totalité de la dépense pour le compte d'un ou plusieurs autres membres, auquel cas il ne doit rien lui-même. Le créateur peut aussi remplacer le calcul automatique par un montant précis pour un ou plusieurs membres ; le reste à répartir est alors partagé également entre les membres restés en calcul automatique.
