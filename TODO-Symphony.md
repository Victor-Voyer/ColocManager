# ColocManager — Checklist de développement

> Projet de fin d'année RNCP 5 — Application web de gestion de colocation  
...
### Tables à créer (via migrations Doctrine)
...
### Mise en place Symfony/Doctrine
- [ ] Configurer la connexion MySQL dans `.env`
- [ ] Créer les entités Doctrine correspondantes
- [ ] Générer et exécuter les migrations
- [ ] Créer les fixtures pour les données de test
...
### Backend (Symfony)
- [ ] Créer l'entité `User` avec les champs nécessaires
- [ ] Configurer `security.yaml` (provider, firewall, encodeur)
- [ ] Intégrer **LexikJWTAuthenticationBundle** pour la gestion des tokens JWT
- [ ] Implémenter l'endpoint `POST /api/register` (inscription)
  - [ ] Validation des champs (email unique, mot de passe fort)
  - [ ] Hash du mot de passe avec `PasswordHasher`
  - [ ] Retour du token JWT à la création
- [ ] Implémenter l'endpoint `POST /api/login` (connexion)
  - [ ] Authentification par email/mot de passe
  - [ ] Retour d'un JWT valide
- [ ] Implémenter l'endpoint `POST /api/logout`
- [ ] Implémenter `GET /api/me` (profil de l'utilisateur connecté)
- [ ] Implémenter `PUT /api/me` (mise à jour du profil)
- [ ] Implémenter `DELETE /api/me` (suppression de compte)
- [ ] Système d'invitation par **lien unique** (token + expiration)
- [ ] Système d'invitation par **email** (envoi via Mailer Symfony)
- [ ] Gestion des rôles : `ROLE_ADMIN_COLOC` / `ROLE_MEMBER`
- [ ] Voter Symfony pour les vérifications de permissions

### Frontend (React)
- [ ] Page d'inscription (`/register`)
- [ ] Page de connexion (`/login`)
- [ ] Stockage du JWT (localStorage ou cookie httpOnly)
...
- [ ] Regénérer le lien d'invitation à la demande (Admin)
...
- [ ] **Système de rotation automatique** :
  ...
  - [ ] Faire en sorte qu'un membre ne puisse pas avoir plusieurs tâches récurente (selon l'importance)
...
## 7. Liste de courses collaborative (task à par entière)

### Backend (Symfony)
- [ ] Implémenter `POST /api/colocations/{id}/shopping` (ajouter un article)
  - [ ] Champs : nom, quantité, unité, assigné_à (tâche récurante)
...
- [ ] Intégrer WebSocket pour les notifications en temps réel
  - [ ] Hub Mercure configuré
  - [ ] Publication d'un événement à chaque nouveau message
  - [ ] Abonnement côté React

### Frontend (React)
- [ ] Page messagerie (`/messages`)
- [ ] Affichage de la conversation (bulles, avatar, horodatage)
- [ ] Formulaire d'envoi de message
- [ ] Mise à jour en temps réel (EventSource / WebSocket)
...
- [ ] Configurer React Router v6 (`react-router-dom`)
...
- [ ] Mettre en place les variables d'environnement (`.env`) pour l'URL de l'API
...
### Backend
- [ ] Hash des mots de passe avec `symfony/password-hasher` (bcrypt/argon2i)
- [ ] Validation de toutes les entrées utilisateur (Assert Symfony / DTOs)
- [ ] Protection contre l'injection SQL via Doctrine ORM (pas de requêtes brutes)
...
- [ ] Expiration des tokens JWT (durée courte + refresh token si besoin)
- [ ] Expiration et invalidation des liens d'invitation
- [ ] Headers de sécurité HTTP (CORS configuré strictement, Content-Security-Policy)
- [ ] Logs des actions sensibles (connexions, suppressions)
...
- [ ] Tests d'intégration des endpoints API (WebTestCase Symfony)
...
- [ ] Configurer **PHPStan** (analyse statique) niveau 5+
- [ ] Configurer **PHP CS Fixer** (style de code PSR-12)
...
- [ ] Mettre en place une **pull request template**
- [ ] Documentation API avec **NelmioApiDocBundle** (OpenAPI/Swagger)
  - [ ] Annoter tous les endpoints avec les paramètres et réponses
  - [ ] Accès à `/api/doc` en développement
...
- [ ] `Dockerfile` pour le backend Symfony (PHP-FPM + Nginx)
...
- [ ] `docker-compose.yml` avec les services :
  ...
  - [ ] `mercure` *(si messagerie implémentée)*
...
- [ ] `docker-compose.prod.yml` avec configuration de production
  - [ ] Mode `APP_ENV=prod` pour Symfony
...
- [ ] Prérequis (Docker, Node, PHP, etc.)
- [ ] Instructions d'installation en développement (clone + `docker-compose up`)
...
- [ ] Swagger/OpenAPI accessible sur `/api/doc`
